...
import {
  Clock, User, CreditCard, Smartphone, Banknote,
  CheckCircle, ChefHat, UtensilsCrossed, DollarSign, XCircle,
} from 'lucide-react';
...
export default function OrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      if (filter === 'active')    query = query.in('status', ['pending', 'confirmed', 'preparing', 'ready']);
      if (filter === 'completed') query = query.in('status', ['served', 'cancelled']);
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      ...
      await supabase.from('order_status_history').insert({
        order_id: orderId, status: newStatus, changed_by: 'restaurant',
      });
      fetchOrders();
    } catch (err) {
  ...
  const updatePaymentStatus = async (orderId: string, newPaymentStatus: string) => {
    try {
      const updateData: Record<string, string> = { payment_status: newPaymentStatus };
      ...
      if (newPaymentStatus === 'paid') {
        await supabase.from('order_status_history').insert({
          order_id: orderId, status: 'confirmed', changed_by: 'restaurant',
        });
      }
      fetchOrders();
    } catch (err) {
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      ...
      await supabase.from('order_status_history').insert({
        order_id: cancelTarget.id, status: 'cancelled', changed_by: 'restaurant',
      });
  ...
  const getNextStatus   = (s: string) => { const i = statusFlow.indexOf(s); return i < statusFlow.length - 1 ? statusFlow[i + 1] : null; };
  ...
  return (
    <DashboardLayout>
        ...
        {(['all', 'active', 'completed'] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-2 rounded-lg font-medium capitalize transition-colors ${
              filter === f ? 'bg-orange-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'
            }`}
          >
            {f}
          </button>
      ...
      {orders.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p className="text-gray-500">
            {filter === 'active' ? 'New orders will appear here automatically' : 'No orders match your filter'}
          </p>
        </div>
      ) : (
              ...
              <div key={order.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    {order.order_items.map((item) => (
                      <div key={item.id} className="flex justify-between items-start">
                        <div className="flex-1">
                          <p className="font-medium text-gray-900">{item.quantity}x {item.menu_items.name}</p>
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-2">
                      {order.payment_method === 'upi'  ? <Smartphone className="w-4 h-4 text-gray-500" /> :
                       order.payment_method === 'card' ? <CreditCard  className="w-4 h-4 text-gray-500" /> :
            ...
            <div className="px-6 py-4">
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-5">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-semibold text-gray-900">Order #{cancelTarget.order_number}</span>
                  <span className="text-sm font-bold text-gray-900">₹{cancelTarget.total.toFixed(2)}</span>
                </div>
                <div className="flex gap-3 text-xs text-gray-500">
                  ...
                  <span>·</span>
                  <span className="capitalize">{cancelTarget.payment_method}</span>
                </div>
      ...
      {/* ── Toast ── */}
      {toast && (
        <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-3 px-5 py-3 rounded-2xl shadow-xl text-sm font-semibold text-white transition-all

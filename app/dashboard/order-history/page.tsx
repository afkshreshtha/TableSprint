...
let query = supabase
  .from("orders")
  .select("id, order_number, table_number, status, payment_method, payment_status, subtotal, tax, service_charge, total, created_at")
...
const exportToCSV = () => {
  ...
  const summary = [
    ...
    ["Lost Revenue (Cancelled)", `₹${lostRevenue.toFixed(2)}`],
    [],
    // ── Orders header ────────────────────────────────────────
    ["ORDER DETAILS"],
    [
  ...
  const rows = filteredOrders.map((o, i) => {
    const subtotal = (o as any).subtotal ?? "";
    const tax      = (o as any).tax ?? "";
    const sc       = (o as any).service_charge ?? "";
    return [
      i + 1,
      o.order_number,
      ...
      subtotal !== "" ? Number(subtotal).toFixed(2) : "N/A",
      tax      !== "" ? Number(tax).toFixed(2)      : "N/A",
      sc       !== "" ? Number(sc).toFixed(2)        : "N/A",
  ...
  const footer = [
    ...
    [],
    ["Report generated by Tabrova Restaurant Management System"],
  ];

  const allRows = [...summary, ...rows, ...footer];
  ...
  const BOM = "\uFEFF"; // UTF-8 BOM so Excel shows ₹ correctly
  const blob = new Blob([BOM + csv], { type: "text/csv;charset=utf-8;" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `orders-${dateLabel.toLowerCase().replace(/ /g, "-")}-${new Date().toISOString().split("T")[0]}.csv`;
  a.click();

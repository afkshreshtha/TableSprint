...
import { ShoppingCart, Plus, Minus, Leaf, ChevronUp, Flame } from "lucide-react";
...
// ── Component ────────────────────────────────────────────────────────────────
export default function TableClient({ restaurant, table, categories, items }: TableClientProps) {
  ...
  const cardShadow = t.cardStyle === "elevated"
    ? `0 2px 18px rgba(0,0,0,${dark ? ".45" : ".09"})`
    : "none";
  const cardBorderV = t.cardStyle === "outlined"
    ? `1px solid ${borderCol}`
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (existingToken && isSame) {
          ...
          if (sessionData) {
            ...
            if (cartItems && Array.isArray(cartItems)) {
              dispatch(setCart(cartItems.map((item: any) => ({
        ...
        const session = await createSession(restaurant.id, table.id, table.table_number);
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: t.bgColor, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <style>{`
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ position: "relative", width: 64, height: 64, margin: "0 auto 24px" }}>
            <div className="lr" style={{
              ...
              borderTopColor: t.accentColor, borderRightColor: t.accentColor,
            }} />
            <div style={{
      ...
      <div className="menu-root">
        ...
        <div className="hero">
          <div style={{ maxWidth: 680, margin: "0 auto" }}>
            ...
            <h1 className="hero-title"><em>{restaurant.name}</em></h1>
            {restaurant.description && <p className="hero-sub">{restaurant.description}</p>}
        ...
        {categories.length > 0 ? (
              ...
              <div key={category.id} ref={(el) => { categoryRefs.current[category.id] = el; }}>
                <div className="section-wrap">
                  <div className="section-head">
                    {category.image_url ? (
                      <img src={category.image_url} alt={category.name} className="section-cat-img" />
                    ) : (
                          ...
                          <div className="item-footer">
                            <div>
                              ...
                              <p className="item-price">
                                <span className="rupee">₹</span>{item.price.toFixed(0)}
                              </p>
                            </div>
                            {!cartItem ? (
                              <button className="btn-add" onClick={() => handleIncrease(item)}>
                                <Plus size={13} strokeWidth={3} /> Add
                              </button>
                            ) : (
                              <div className="counter-wrap">
                                <button className="counter-btn minus" onClick={() => handleDecrease(item)}>
                                  <Minus size={12} strokeWidth={3} />
                                </button>
                                <span className="counter-num">{cartItem.quantity}</span>
                                <button className="counter-btn" onClick={() => handleIncrease(item)}>
          ...
          <div style={{ textAlign: "center", padding: "80px 20px", color: dimText }}>
            <p style={{ fontSize: 48, marginBottom: 16 }}>🍽️</p>
            <p style={{ fontFamily: `'${fp.display}', serif`, fontSize: 20 }}>No menu items yet</p>
          </div>
        )}
            ...
            >
              <div className="cart-left">
                ...
                <div style={{ textAlign: "left" }}>
                  ...
                  <p className="cart-items">{totalItems} {totalItems === 1 ? "item" : "items"}</p>
                </div>
              </div>
              <p className="cart-total">
                <span className="rupee">₹</span>{cart.total?.toFixed(0) ?? "0"}
              </p>
            </button>
          </div>
        )}

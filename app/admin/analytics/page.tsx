...
export default function AnalyticsPage() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const subStats = subscriptions?.reduce((acc, sub) => {
        if (sub.status === 'active') acc.active++;
        else if (sub.status === 'trialing') acc.trial++;
        else if (sub.status === 'cancelled') acc.cancelled++;
        if (sub.subscription_plans?.name === 'free') acc.free++;
        return acc;
      }, { active: 0, trial: 0, free: 0, cancelled: 0 }) || { active: 0, trial: 0, free: 0, cancelled: 0 };
      ...
      setAnalytics({
        revenue: {
          today: todayPayments?.reduce((sum, p) => sum + p.amount, 0) / 100 || 0,
          yesterday: yesterdayPayments?.reduce((sum, p) => sum + p.amount, 0) / 100 || 0,
          thisMonth: thisMonthPayments?.reduce((sum, p) => sum + p.amount, 0) / 100 || 0,
          lastMonth: lastMonthPayments?.reduce((sum, p) => sum + p.amount, 0) / 100 || 0,
          total: allPayments?.reduce((sum, p) => sum + p.amount, 0) / 100 || 0,
        },
  ...
  const revenueChange = analytics.revenue.yesterday > 0
    ? ((analytics.revenue.today - analytics.revenue.yesterday) / analytics.revenue.yesterday) * 100
    : 0;

  const monthlyChange = analytics.revenue.lastMonth > 0
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Restaurant Growth</h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={growthChartData}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="restaurants" fill="#ea580c" name="New Restaurants" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
